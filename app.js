...
// Trainer UI Elemente
const trainerNav = document.querySelector('.trainer-nav');
const trainerAllResultsList = document.getElementById('trainer-all-results-list');
...
// Profil Customization Elemente
const avatarWrapper = document.getElementById('profile-avatar-trigger');
const avatarUpload = document.getElementById('avatar-upload');
const avatarDisplay = document.getElementById('profile-avatar-display');
const btnEditName = document.getElementById('btn-edit-name');
const btnSaveName = document.getElementById('btn-save-name');
const nameDisplayGroup = document.querySelector('.editable-name-container');
...
async function deleteEntry(id, name) {
    if (!isTrainer) {
        showToast("Nur der Trainer kann Ergebnisse löschen!", "error");
        return;
    }
...
onAuthStateChanged(auth, (user) => {
    ...
    document.querySelectorAll('.trainer-only').forEach(el => {
        if (isTrainer) el.classList.remove('hidden');
        else el.classList.add('hidden');
    });
    
    if (isTrainer) trainerNav.classList.remove('hidden');
    else trainerNav.classList.add('hidden');
    ...
    if (user) {
        ...
        navProfileText.textContent = 'Profil';
        headerBtnLogout.classList.remove('hidden');
        headerBtnLogin.classList.add('hidden');
        ...
        if (user.photoURL) {
            ...
            avatarDisplay.src = 'icons/wappen.jpg'; // Fallback
        }
        ...
        headerBtnLogout.classList.add('hidden');
        headerBtnLogin.classList.remove('hidden');
...
headerBtnLogout.addEventListener('click', async () => {
    ...
    try {
        ...
        // Wechsle zum Ranking nach Logout
        document.querySelector('[data-target="view-leaderboard"]').click();
    } catch (error) {
...
function renderTrainerDashboard(filter = "") {
    trainerAllResultsList.innerHTML = '';
    
    // Wir nutzen hier direkt die leaderboardData, die durch den onSnapshot aktuell gehalten wird
    let data = [...leaderboardData].sort((a, b) => new Date(b.date) - new Date(a.date));
    
    if (filter) {
        data = data.filter(entry => entry.name.toLowerCase().includes(filter.toLowerCase()));
    }

    if (data.length === 0) {
        trainerAllResultsList.innerHTML = '<p style="text-align:center; color: var(--text-muted); padding: 1rem;">Keine Ergebnisse gefunden.</p>';
        return;
    }
    ...
    data.forEach(entry => {
        ...
        li.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center;">
                <span style="font-weight: 800; color: var(--primary);">${entry.score} Ringe</span>
                <span style="font-size: 0.8rem; color: var(--text-muted)">${formatDate(entry.date)}</span>
            </div>
            <div style="display:flex; justify-content:space-between; align-items:center;">
                <span style="font-weight: 600;">${entry.name}</span>
                <button class="btn-delete-entry" data-id="${entry.id}" style="padding: 2px;">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2M10 11v6M14 11v6"/></svg>
                </button>
            </div>
            <div style="font-size: 0.75rem; color: var(--text-muted)">${seriesText}</div>
        `;

        li.querySelector('.btn-delete-entry').addEventListener('click', (e) => {
            e.stopPropagation();
            deleteEntry(entry.id, entry.name);
            // Re-render Trainer view after a short delay
            setTimeout(() => renderTrainerDashboard(trainerSearch.value), 1000);
        });

        trainerAllResultsList.appendChild(li);
    });
}

trainerSearch.addEventListener('input', (e) => {
    renderTrainerDashboard(e.target.value);
});
...
btnEditName.addEventListener('click', () => {
    nameDisplayGroup.classList.add('hidden');
    nameEditGroup.classList.remove('hidden');
    nameEditGroup.style.display = 'flex';
...
btnSaveName.addEventListener('click', async () => {
    ...
    try {
        ...
        profileNameDisplay.innerHTML = newName + (isTrainer ? ' <span class="trainer-only-badge">TRAINER</span>' : '');
        
        nameDisplayGroup.classList.remove('hidden');
        nameEditGroup.classList.add('hidden');
        nameEditGroup.style.display = 'none';
        
        showToast('Name aktualisiert!');
        renderLeaderboard(); // Damit der Name in der Liste (wenn vorhanden) auch stimmt
    } catch (error) {
